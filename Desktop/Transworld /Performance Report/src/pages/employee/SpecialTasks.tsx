...
export function SpecialTasks() {
  ...
  const [teamSortKey,  setTeamSortKey]    = useState<TeamTaskSortKey>('due_date')
  ...
  const sortedTeam = useMemo(() => {
    return [...filteredTeam].sort((a, b) => {
      ...
      if (teamSortKey === 'assigned_to') {
        const na = (a.assignees ?? []).map((x) => profiles.find((p) => p.id === x.employee_id)?.full_name ?? '').join(',')
        const nb = (b.assignees ?? []).map((x) => profiles.find((p) => p.id === x.employee_id)?.full_name ?? '').join(',')
        return sign * na.localeCompare(nb)
      }
      ...
      if (teamSortKey === 'created_at') return sign * ((a.created_at ?? '') < (b.created_at ?? '') ? -1 : 1)
  ...
  }, [filteredTeam, teamSortKey, teamSortDir, profiles])
            ...
            <>
              ...
              <div className="divide-y divide-slate-100 sm:hidden">
                {sortedTeam.map((task) => {
                  ...
                  const assignedBy = profiles.find((p) => p.id === task.assigned_by)
                  const today2 = new Date().toISOString().slice(0, 10)
                  const isOverdue = task.due_date && task.due_date < today2 && task.status !== 'Completed' && task.status !== 'Acknowledged'
                  return (
                    <div key={task.id} onClick={() => setSelectedDetail({ kind: 'st', data: task })} className="px-4 py-3 hover:bg-slate-50/70 cursor-pointer active:bg-slate-100">
                      <div className="flex items-start justify-between gap-2">
                        <p className={cn('text-sm font-medium leading-snug flex-1', task.status === 'Completed' ? 'line-through text-slate-400' : 'text-slate-800')}>{task.task_name}</p>
                        <span className={cn('shrink-0 rounded-full px-2 py-0.5 text-[10px] font-semibold whitespace-nowrap', STATUS_COLORS[task.status])}>{STATUS_LABELS[task.status]}</span>
                      </div>
                      <div className="mt-1.5 flex items-center gap-2">
                        ...
                        {assignees.length > 2 && <span className="text-xs text-slate-400">+{assignees.length - 2} more</span>}
                      </div>
                      <div className="mt-1 flex flex-wrap gap-x-3 gap-y-0.5 text-xs text-slate-400">
                        {assignedBy && <span>By {assignedBy.full_name}</span>}
                        {task.due_date && (
